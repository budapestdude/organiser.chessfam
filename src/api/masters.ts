...
export const mastersApi = {
  ...
  // Event availability: Update master's event availability
  updateEventAvailability: async (data: {
    ...
    const response = await apiClient.put('/masters/event-availability', data);
    return response.data;
  },

  // Event availability: Get masters available for events
  getMastersAvailableForEvents: async (params?: {
    event_type?: string;
    min_rating?: number;
    max_rate?: number;
    page?: number;
    ...
    const response = await apiClient.get('/masters/available-for-events', { params });
    return response.data;
  },
};
