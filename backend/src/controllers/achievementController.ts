...
export const getUserStats = async (req: Request, res: Response) => {
  try {
    ...
    // Get or create user stats (select all columns to handle schema changes gracefully)
  ...
  try {
    ...
    for (const achievementKey of relevantAchievements) {
      ...
      const achievementResult = await pool.query(
        'SELECT id, achievement_key, name, description, icon, category, tier, requirement_value FROM achievements WHERE achievement_key = $1',
        [achievementKey]
      );
      ...
      const userAchievementResult = await pool.query(
        'SELECT id, user_id, achievement_id, progress, unlocked_at FROM user_achievements WHERE user_id = $1 AND achievement_id = $2',
