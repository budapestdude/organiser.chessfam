...
import { favoritesApi, type ItemType as ApiItemType } from '../api/favorites';
...
interface AppState {
  ...
  removeFavorite: (itemId: number, type: Favorite['type']) => Promise<void>;
    ...
    (set, get) => ({
      ...
      login: async (email, password) => {
        ...
        try {
          ...
          // Fetch user's favorites after login
          get().fetchFavorites();
        } catch (error: any) {
          set({
      ...
      signup: async (name, email, password) => {
        ...
        try {
          ...
          TokenManager.setTokens(token, refreshToken);
          set({ user, isLoading: false, isAuthModalOpen: false });
          // Fetch user's favorites after signup (will be empty, but sets up state)
          get().fetchFavorites();
        } catch (error: any) {
          set({
      ...
      initializeAuth: async () => {
        ...
        if (token && !TokenManager.isTokenExpired(token)) {
          try {
            ...
            set({ user });
            // Fetch user's favorites after authentication
            get().fetchFavorites();
          } catch (error) {
      ...
      // Favorite actions - now synced with backend API
      fetchFavorites: async () => {
        try {
          const response = await favoritesApi.getFavorites(undefined, false);
          const favorites: Favorite[] = response.data.map((fav: any) => ({
            id: String(fav.id),
            type: fav.item_type === 'venue' ? 'location' : fav.item_type,
            itemId: fav.item_id,
            itemName: fav.item?.name || '',
            itemImage: fav.item?.image || '',
      ...
      addFavorite: async (favorite) => {
        // Map 'location' to 'venue' for backend
        const apiType: ApiItemType = favorite.type === 'location' ? 'venue' : favorite.type as ApiItemType;

        try {
          // Call API first
          await favoritesApi.addFavorite(apiType, favorite.itemId);
      ...
      removeFavorite: async (itemId, type) => {
        // Map 'location' to 'venue' for backend
        const apiType: ApiItemType = type === 'location' ? 'venue' : type as ApiItemType;

        try {
          // Call API first
          await favoritesApi.removeFavorite(apiType, itemId);
        } catch (error) {
