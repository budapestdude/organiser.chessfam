...
export const createBooking = async (req: Request, res: Response) => {
  try {
    ...
    // Require identity verification for creating bookings
    try {
    ...
    const masterCheck = await pool.query(
      'SELECT id, name, title, rating, price_rapid, price_blitz, price_bullet, price_classical, available FROM masters WHERE id = $1 AND available = true',
      [master_id]
    );
