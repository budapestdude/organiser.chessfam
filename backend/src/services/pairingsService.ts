 ...
 *
 * TRF Format Specification:
 * Line 012 (Tournament Header):
 *   Positions 1-3: "012"
 *   Position 4: Space
 *   Positions 5-88: Tournament name (84 characters)
 *   Positions 89-98: Date in YYYY/MM/DD format (10 characters)
 *
 * Line XXR (Number of Rounds):
 *   Positions 1-3: "XXR"
 *   Position 4: Space
 *   Positions 5+: Number of rounds played
 *
 * Line 001 (Player Data):
 *   Positions 1-3: "001"
 *   Position 4: Space
 *   Positions 5-8: Starting rank (4 digits, right-aligned)
 *   Positions 9-14: Spacing (6 spaces)
 *   Positions 15-47: Player name (33 characters, left-aligned)
 *   Position 48: Space
 *   Positions 49-52: Rating (4 digits, right-aligned)
 *   Positions 53-88: Optional fields (36 spaces)
 *   Positions 89-93: Total points (5 chars including decimal, right-aligned)
 *   Position 94: Space
 *   Positions 95-98: Rank (4 digits, right-aligned)
 *   Positions 99+: Round results (8 chars per round)
 */
export async function generateTRFFile(tournamentId: number, roundNumber: number): Promise<string> {
  ...
  // FIX 1: Use YYYY/MM/DD format (not YYYYMMDD)
  // FIX 2: Pad tournament name to 84 characters (not 80)
  const date = new Date().toISOString().split('T')[0].replace(/-/g, '/');
  trf += `012 ${tournament.name.padEnd(84, ' ')}${date}\n`;
  ...
  for (const player of players) {
    ...
    // FIX 3: Use 36 spaces between rating and score (not 22)
    // This ensures score starts at position 89 and rank at position 95
    trf += `001 ${pairingNum}      ${name} ${rating}${''.padEnd(36, ' ')}${scoreStr} ${rank}${resultsStr}\n`;
  }
...
function parseTRFPairings(trfContent: string, roundNumber: number): Pairing[] {
  ...
  for (const line of lines) {
    if (line.startsWith('001')) {
      ...
      // Results start at position 99 (0-indexed: 98), each result takes 8 characters
      const resultsStart = 98;
